...
class NLIMemo:
    """Append-only JSONL cache for NLI scores."""
...
def _hash256(s: str) -> str:
    """MD5 of first 256 UTF-8 chars."""
    return hashlib.md5(s[:256].encode("utf-8")).hexdigest()

# Strong lexical triggers (core political/issue cues; Traditional+Simplified strings)
STRONG_KEYS = [
    # cross-strait / identity / external
    ...
    # elections / legislature / procedure
    "大選","總統","立法院","立委","政見","辯論","投票","不分區","三讀","質詢",
    # national symbols / memory politics
    "國旗","國歌","國徽","國慶","升旗","轉型正義","二二八","歷史正義","集體記憶","共同體","國格"
]

# Exclude homographs for 統一/统一 (invoice, 7-Eleven, Uni-President)
ANTI_KEYS = ["統一發票","统一发票","統一超商","统一超商","統一企業","统一企业"]

def hit_strong_keys(s: str) -> bool:
    """True if any STRONG_KEYS substring matches and ANTI_KEYS do not."""
    s = s or ""
...
class TextFilterStack:
    ...
    def _build_whitelist(self) -> Set[str]:
        """Actors (people/parties) in actor_terms; institutions/issues in inst_terms (mixed scripts)."""
        # People / parties / camps
        self.actor_terms = {
...
# Runner functions (process_single_file, batch_process_datasets) have been
# moved to s02_filter.py.  This module is now a pure utility library.
